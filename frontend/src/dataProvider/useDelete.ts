...
>(
    ...
    const updateCache = ({ resource, id }: { resource: any; id: any; }) => {
        ...
        const updater: Updater<GetListResult, any> = (res: GetListResult) => {
            if (!res || !res.data) return res;
            const newCollection = updateColl(res.data);
            const recordWasFound= (newCollection && newCollection.length < res.data.length) ?? false;
        ...
        const updaterInfinite: Updater<UseInfiniteQueryResult<GetInfiniteListResult>['data'], any> = (res: UseInfiniteQueryResult<GetInfiniteListResult>['data'])=> {
                ...
                return {
                    ...
                    pages: res.pages.map(page => {
                        ...
                        const recordWasFound =
                          (newCollection && newCollection.length < page.data.length) ?? false;
                        return recordWasFound
                            ? {
                                  ...page,
                                  data: newCollection,
        ...
        queryClient.setQueriesData(
            [resource, 'getMany'],
            ((coll: RecordType[]) =>
                coll && coll.length > 0 ? updateColl(coll) : coll) as Updater<RecordType[], any>,
            { updatedAt }
        );
        const updaterManyRef: Updater<GetListResult, any> = (res: GetListResult) => {
                if (!res || !res.data) return res;
                const newCollection = updateColl(res.data);
                const recordWasFound = (newCollection && newCollection.length < res.data.length) ?? false;
                return recordWasFound
  ...
  const mutation = useMutation<
    ...
    const mutate = async (
        ...
        if (reactMutationOptions.onSuccess) {
            const { onSuccess } = reactMutationOptions;
            setTimeout(
                () =>
                    onSuccess(
                        callTimePreviousData as RecordType,
